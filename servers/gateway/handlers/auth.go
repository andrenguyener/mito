...
func (ctx *Context) UsersHandler(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		user, err := ctx.UserStore.GetByEmail(newUser.Email)
		if err != users.ErrUserNotFound && user != nil {
			http.Error(w, "Error email already exists: "+newUser.Email, http.StatusBadRequest)
			return
		}
...
func (ctx *Context) UsersMeHandler(w http.ResponseWriter, r *http.Request) {
	...
	case "PATCH":
		...
		err = ctx.UserStore.Update(sessionUser.ID, userUpdates)
		...
		ctx.TrieStore.Remove(sessionState.User.FirstName, sessionState.User.ID)
		ctx.TrieStore.Remove(sessionState.User.LastName, sessionState.User.ID)

		// update the session state with the user
		sessionState.User.FirstName = userUpdates.FirstName
		sessionState.User.LastName = userUpdates.LastName
		...
		ctx.TrieStore.Insert(sessionState.User.FirstName, sessionState.User.ID)
		ctx.TrieStore.Insert(sessionState.User.LastName, sessionState.User.ID)
